...
$typesIncludePath = Join-Path $CorePath "Sources/Interop/CTsubameABITypes/include"
...
& clang -std=c11 -Wall -Wextra -Werror `
    "-I$includePath" "-I$typesIncludePath" -c $sourcePath -o $objectPath
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& clang++ -std=c++17 -Wall -Wextra -Werror `
    "-I$includePath" "-I$typesIncludePath" -x c++ -fsyntax-only $sourcePath
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not ($exports -match "Name: tsubame_lookup")) {
    throw "TsubameCoreABI.dll does not export tsubame_lookup."
}
